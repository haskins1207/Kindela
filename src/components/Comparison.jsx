...
const COLS = [
  { name: 'Civora', ex: 'all-in-one', civora: true },
...
// Honest scoring from civora-competitive-landscape.md. Order: Civora, Donor CRM, Retail POS, Resale POS, Enterprise, Spreadsheets.
const ROWS = [
...
// "Why not just use X?" — honest one-liners (from civora-differentiators.md). Headings name products for search intent.
const ONE_LINERS = [
  ['ThriftCart', 'ThriftCart runs your resale floor. Civora runs your whole nonprofit — donors, grants, books, volunteers, and a public report — with the resale floor connected to all of it.'],
  ['a donor CRM (Bloomerang, DonorPerfect)', 'Great donor CRMs — but they don’t run your resale store or your books. Civora connects the gift to the good to the ledger to the public report.'],
  ...
  ['Zeffy', 'Zeffy is a great free start. When the spreadsheets and logins pile up, Civora is what you graduate into — one connected system.'],
    ...
    <section className="section compare" id="compare">
      <div className="container container-wide">
        <Reveal className="section-head">
          ...
          <p>Each tool does one slice. Civora connects them — and adds the parts none of them have.</p>
        </Reveal>

        <Reveal className="compare__table-wrap">
          <table className="compare__table">
            <caption className="sr-only">
              How Civora compares to donor CRMs, retail POS, resale POS, enterprise suites, and spreadsheets
            </caption>
            <thead>
              <tr>
                <th scope="col" className="compare__rowhead-blank"><span className="sr-only">Capability</span></th>
                {COLS.map((c) => (
                  <th key={c.name} scope="col" className={c.civora ? 'compare__col compare__col--civora' : 'compare__col'}>
            ...
            <tbody>
              {ROWS.map((row) => (
                <tr key={row.cap}>
                  ...
                  {row.vals.map((v, i) => (
                    <td key={i} className={i === 0 ? 'compare__td compare__td--civora' : 'compare__td'}>
                      <Cell value={v} />
                    </td>
                  ))}
                </tr>
        ...
        <p className="compare__note">
          Based on Civora’s 2026 competitive research. <strong>Partial</strong> = limited, available only via an add-on
